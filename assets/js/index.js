...
async function fetchPokemons() {
    const response = await fetch("https://pokeapi.co/api/v2/pokemon?limit=20&offset=1");
...
function insertPokemonHtml(pokemon) {
    ...
    <li class="pokemons-item">
        <div class="pokemons-card" data-pokemon-car-type-name="Electric">
            <div class="pokemons-card-image-container">
                <img src="${pokemon.sprites.other.dream_world.front_default}" alt="" class="pokemon-card-image">
            </div>
...
async function filterClicked(filter, pokemonsData) {
    ...
    const pokemonsDataFilterByType = pokemonsData.filter(pokemonData => {
...
function initFiltersFunction(pokemonsData){
    ...
    filters.forEach(filter => {
        filter.addEventListener('click', () =>  filterClicked(filter, pokemonsData))
    })
}
